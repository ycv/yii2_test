...
(function (root, factory) {
    ...
    ChineseRegion.prototype.getAreasByParentId = function (id) {
        ...
        for (var i = 0; i < this._source.length; i++) {
            if (this._source[i].level > this.maxLevel || this._source[i].level < this.minLevel)
                continue;
            if (this._source[i].parentId != id)
                continue;
    ...
    ChineseRegion.prototype.getCurrentAreas = function (lastId) {
        ...
        for (var i = this.minLevel; i <= this.maxLevel; i++) {
            ...
            if (area === undefined)
                break;
            if (area.level > this.maxLevel || area.level < this.minLevel)
                continue;
            result.unshift(area);
    ...
    $.fn.chineseRegion = function () {
        ...
        if (chineseRegion === undefined)
            this.data('bs-chinese-region', (chineseRegion = new ChineseRegion(this)));
        if (typeof options == 'string')
            chineseRegion[options].apply(chineseRegion, arg);
        return this;
    };
